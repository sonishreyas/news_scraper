...
class NewsSpider(scrapy.Spider):
    ...
    def __init__(self, source_information):
        ...
        self.source_is_article_url = source_information.get("source_is_article_url")
        self.article_url           = source_information.get("article_url")

    def start_requests(self):
        if self.source_is_article_url:
            print("request is for article with url " + self.article_url)
            yield scrapy.Request(url = self.article_url, callback = self.parse_news_articles)
        else:
    ...
    def save(self,summarized_news):
        print("Saving the records", summarized_news)
        mydb = MongoDB()
        object_id = mydb.insert_many("articles",summarized_news)
        #print("inserted into db", object_id)
